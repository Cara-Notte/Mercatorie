...
def train_and_save(raw_data_path=RAW_DATA_PATH, horizons: tuple[int, ...] = SUPPORTED_HORIZONS, stable_band: float = 1.0) -> None:
    ...
    for horizon in horizons:
        ...
        preprocessor_path = preprocessor_artifact_path(horizon)
        artifact_name = artifact_path.name

        joblib.dump(pipeline, artifact_path)
        joblib.dump(pipeline.named_steps["preprocess"], preprocessor_path)
        ...
        metadata = {
            ...
            "preprocessor_artifact_name": preprocessor_path.name,
            "metrics": metrics,
        }
        ...
        LOGGER.info("Saved %sd preprocessor artifact: %s", horizon, preprocessor_path)
